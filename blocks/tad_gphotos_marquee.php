...
function tad_gphotos_marquee($options)
{
    ...
    $where = !empty($album_sn) ? 'WHERE `album_sn` = ?' : 'ORDER BY `create_date` DESC LIMIT 0,1';
    $sql = 'SELECT `album_sn`, `album_url`, `album_name` FROM `' . $xoopsDB->prefix('tad_gphotos') . '` ' . $where;
    $result = !empty($album_sn) ? Utility::query($sql, 'i', [$album_sn]) : Utility::query($sql) or Utility::web_error($sql, __FILE__, __LINE__);
